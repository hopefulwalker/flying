 ...
 * Generation time is Sat Jun 03 00:45:01 CST 2017
 */
...
public class AccountMsgCodec implements IAccountMsgCodec {
    public byte[] encodeGetAccountByIdReply(long requestNo,int retCode,AccountBO accountBO){
        ...
        bodyEncoder.wrap(buffer, headerEncoder.encodedLength()).requestNo(requestNo).retCode(retCode).aid(accountBO.getAid()).type(accountBO.getType()).custId(accountBO.getCustId()).parentId(accountBO.getParentId()).statusId(accountBO.getStatusId()).createTime(accountBO.getCreateTime()).updateTime(accountBO.getUpdateTime());
        
        return replyBytes;
    }
    public AccountBO getGetAccountByIdReply(byte[] bytes){
    ...
    public byte[] encodeGetAccountByIdRequest(long requestNo,long aid){
        // headerEncoderClass, varHeaderEncoder, bodyEncoderClass, varBodyEncoder, msgTypeClass, bodyClass, varFields
        // elementExists, elementEncoderClass, varElementEncoder, elementClass, varElements, varElement, varElementFields
        MessageHeaderEncoder headerEncoder = new MessageHeaderEncoder();
        ...
        bodyEncoder.wrap(buffer, headerEncoder.encodedLength()).requestNo(requestNo).aid(aid);
    ...
    public GetAccountByIdReplyDecoder getGetAccountByIdReplyDecoder(byte[] msg){
        // headerDecoderClass, varHeaderDecoder, varMsg, bodyDecoderClass, varBodyDecoder
        MessageHeaderDecoder headerDecoder = new MessageHeaderDecoder();
        UnsafeBuffer buffer = new UnsafeBuffer(msg);
        headerDecoder.wrap(buffer, 0);
